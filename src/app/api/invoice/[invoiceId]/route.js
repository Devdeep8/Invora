...
const dueDate2 = data.dueDate
  ? new Date(new Date(createdDate).setDate(createdDate.getDate() + parseInt(data.dueDate)))
  : new Date();
  ...
  if (templateData.yourCompany.address) {
    const addressLines = pdf.splitTextToSize(String(templateData.yourCompany.address), pageWidth/3);
  ...
  if (templateData.billTo.address) {
    const addressLines = pdf.splitTextToSize(String(templateData.billTo.address), pageWidth/2 - leftMargin);
    pdf.text(addressLines, leftMargin, yPosition);
    yPosition += addressLines.length * 5;
  }
  ...
  const tableRows = templateData.items.map(item => [
    ...
    formatCurrency(Number(item.amount || 0) * Number(item.quantity || 0), templateData.currency || "INR")
  ]);
  ...
  autoTable(pdf, {
    ...
    body: [
      ["Sub Total", formatCurrency(Number(templateData.subTotal || 0), templateData.currency || "INR")],
      ...(Number(templateData.taxPercentage || 0) > 0 ? [
        [`Tax (${Number(templateData.taxPercentage || 0)}%)`, formatCurrency(Number(templateData.taxAmount || 0), templateData.currency || "INR")]
      ] : []),
      ["Total Due Amount", formatCurrency(Number(templateData.grandTotal || 0), templateData.currency || "INR")]
    ],
    theme: "grid",
    styles: {
      font: "helvetica",
    ...
    didDrawCell: function(data) {
      ...
      if (data.row.index === (Number(templateData.taxPercentage || 0) > 0 ? 2 : 1)) {
        ...
        pdf.text(cellText, data.cell.x + data.cell.width - 2, data.cell.y + data.cell.height/2, { 
          align: 'right',
          baseline: 'middle'
        });
      }
  ...
  pdf.setFontSize(9);
  pdf.text("Thank you for your business!", pageWidth / 2, footerY + 6, { align: 'center' });
